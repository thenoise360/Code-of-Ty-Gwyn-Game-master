...
def bedStart(deskStatus):
	...
	while (eb ==1 ):
		if action == "go to desk":
			desk(action, deskStatus)
			break
		elif action == "go to bed":
			bed(action, deskStatus)
			break
		elif action == "go to start":
			bedStart(action, deskStatus)
			break
		elif action == "go to sink":
			sink(action, deskStatus)
			break
		elif action == "go to shelf":
			shelf(action, deskStatus)
			break
		elif action == "go to wardrobe":
			wardrobe(action, deskStatus)
			break
		elif action == "go to door":
			door(action, deskStatus)
			break
		elif action == "help":
...
def desk(action, deskStatus):
	...
	if deskStatus == "complete":
		...
		deskNest2(action, deskStatus)

def deskNest1(action, deskStatus):
...
def deskNest2(action, deskStatus):
...
def bed(action, deskStatus):
  print ("Bed")

#SINK BLOCK

def sink(action, deskStatus):
  print ("Sink")

#SHELF BLOCK

def shelf(action, deskStatus):
  print ("Shelf")

#WARDROBE BLOCK

def wardrobe(action, deskStatus):
  print ("Wardrobe")

#DOOR BLOCK

def door(action, deskStatus):
  print ("Door")
...
while True:
    ...
    #.split(action, deskStatus) breaks it up into a list array
